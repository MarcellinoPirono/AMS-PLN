...
function reindex(){
    ...
    ids.forEach( (e, i) => { e.innerHTML="<strong>"+(i+1)+"</strong>" } );
    const ids2 = document.querySelectorAll("tr > td:nth-child(2)");
    ids2.forEach( (e, i) => { e.innerHTML=("<select class='form-control input-default' id='kategory_id["+(i+1)+"]' name='kategory_id["+(i+1)+"]'><option value="+0+" selected disabled>Pilih Kategori</option></select>") } );
    const ids3 = document.querySelectorAll("tr > td:nth-child(3)");
    ids3.forEach( (e, i) => { e.innerHTML=("<select class='form-control input-default' id='item_id["+(i+1)+"]' name='item_id["+(i+1)+"]'><option value="+0+" selected disabled>Pilih Pekerjaan</option></select>") } );
    const ids4 = document.querySelectorAll("tr > td:nth-child(4)");
    ids4.forEach( (e, i) => { e.innerHTML=("<input type='number' class='form-control satuan @error('satuan') is-invalid @enderror' id='satuan["+(i+1)+"]' name='satuan["+(i+1)+"]' placeholder='Satuan' readonly disabled>") } );
    const ids5 = document.querySelectorAll("tr > td:nth-child(5)");
    ids5.forEach( (e, i) => { e.innerHTML=("<input type='number' class='form-control volume @error('volume') is-invalid @enderror' id='volume["+(i+1)+"]' name='volume["+(i+1)+"]' placeholder='Volume' required>") } );
    const ids6 = document.querySelectorAll("tr > td:nth-child(6)");
    ids6.forEach( (e, i) => { e.innerHTML=("<input type='number' class='form-control harga_satuan @error('harga_satuan') is-invalid @enderror' id='harga_satuan["+(i+1)+"]' name='harga_satuan["+(i+1)+"]' placeholder='Harga Satuan' readonly disabled>") } );
    const ids7 = document.querySelectorAll("tr > td:nth-child(7)");
    ids7.forEach( (e, i) => { e.innerHTML=("<input type='number' class='form-control harga @error('harga') is-invalid @enderror' id='harga["+(i+1)+"]' name='harga["+(i+1)+"]' placeholder='Harga' readonly disabled>") } );
    // console.log();
...
function updateform(data_items){
